...
class DTLearner(object):
    ...
    def build_tree(self, data):
        ...
        if (data.shape[0] <= self.leaf_size) or (np.unique(data[:,-1]).size == 1):
            new_leaf = np.array([[-99, data[:,-1].mean(), np.nan, np.nan]])
            return new_leaf
        else:
            corr = []
            for col in range(0, dataX.shape[1]):
                corr.append(abs(np.corrcoef(dataX.T[col], dataY.T)[0][1]))
            factor = np.argmax(corr)
            splitVal = np.median(data[:,factor])

            #left branch of sub-tree
            left_tree_data = data[data[:,factor]<=splitVal]
            #print "left tree data\n", left_tree_data
            # After split data by median, 1. if all data passed to left_tree: this is leaf, return
            # else, valid split 2. if left_tree is one row

            if left_tree_data.shape[0] == data.shape[0]:
                return np.array([[-99, data[:,-1].mean(), np.nan, np.nan]])
            elif left_tree_data.shape[0] <= self.leaf_size:
                left_tree = np.array([[-99, left_tree_data[:,-1].mean(), np.nan, np.nan]])
            else:
            ...
            if right_tree_data.shape[0] == data.shape[0]:
                ...
                right_tree = np.array([[-99, right_tree_data[:,-1].mean(), np.nan, np.nan]])
            else:
    ...
    def query(self,X):
        ...
        @returns the predicted value Y from DTlearner.
        """
