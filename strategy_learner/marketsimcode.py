...
Update: instead of hardcode "JPM", change to sym as an input
Update: trades_df --> trades_delta_df
"""
...
#compute portval based on trade_delta_df
def compute_portvals_trade_delta(trades_delta_df, start_val = 100000, commission=0, impact=0.005, symbol = "JPM"):
    ...
    for my_date, row in trades_delta_df.iterrows():
        my_symbol = [symbol]
    ...
    #initialize
    holdings_df = trades_delta_df.copy()
    holdings_df.set_value(dates[0], '_Cash', start_val)
    holdings_df.set_value(dates[0], symbol, 0)
    # update the 1st row
    holdings_df.iloc[0, 1] += trades_delta_df.iloc[0, 1]
    holdings_df.iloc[0, 0] += trades_delta_df.iloc[0, 0]
    ...
    for i in range(1,len(holdings_df.index)):
        ...
        holdings_df.iloc[i, 0] = (holdings_df.iloc[i-1, 0] + trades_delta_df.iloc[i, 0])
...
def compute_portvals(trades_df, start_val = 100000, commission=0, impact=0.005, symbol = "JPM"):
    ...
    # update the rest of rows in holdings_df '_Cash' column
    for i in range(1,len(holdings_df.index)):
        holdings_df.iloc[i, 1] += (holdings_df.iloc[i-1, 1] + trades_delta_df.iloc[i, 1]) ## potential problem
    #print holdings_df
...
def gen_plot(trades_df, portvals, chart_title, symbol):
